    ...
    def select(self, base_kernel: str, kernel_transforms: List[Tuple[str, List]], selection_method: str,
        ...
        if self.prior_kernel_list is not None:
            ...
            if self.integrate_mode == 'mean':
                ...
                # ------------------------------------------------------------------
                # Added for the "can adaptive prior weighting help?" experiment.
                # See notes/analysis-findings.md. All three fit weights on the
                # LABELLED subset only (train_gold), so they are deployable
                # online; no validation/test labels are used.
                #   softmax_align : softmax over kernel alignment (keeps weak
                #                   priors alive, unlike 'alignment' which is
                #                   a plain normalisation)
                #   best_align    : one-hot on the best-aligned kernel
                #   ridge_lab     : non-negative ridge regression of the truth
                #                   sub-kernel on the kernel bank, with the same
                #                   shrinkage target as the uniform operator
                # ------------------------------------------------------------------
                if self.valid_perts is not None:
