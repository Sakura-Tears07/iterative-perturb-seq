...
class PertData:
    ...
    def load(self, data_name = None, data_path = None):
        ...
        self.ctrl_mean = np.mean(self.ctrl_adata.X, axis = 0).reshape(-1)

        if 'batch' in self.adata.obs.columns:
            print('batch info is available!')
            self.all_batch_idx = np.sort(np.array(self.adata.obs.batch.unique()))
        ...
        if 'pert_idx' not in vars(self.dataset_processed['ctrl'][0])['_store']:
            print('Loading gears 0.0.1 data loaders, processing...')
            for i,j in self.dataset_processed.items():
                pert_idx = self.get_pert_idx(i, self.adata)
                if pert_idx is None:
                    pert_idx = [-1]
                for x in j:
                    x.pert_idx = pert_idx
    ...
    def get_dataloader_from_pert_list(self, pert_list, batch_size, 
        ...
        print('Number of cores: ' + str(os.cpu_count()))
        print('Number of workers: ' + str(num_workers))
    
        if get_distinct_perts:
            for p in pert_list:
                if batch_exp:
            ...
            for p in pert_list:
                if batch_exp:
                    found = False
                    for round, perts in batch_exp['genes_available_per_round'].items():
                        if p in perts:
                            batch_idx_round = batch_exp['batch_idx'][round]
                            print('pert', p , ': len of all batch cells', len(self.dataset_processed[p]))
                            data_load = [i for i in self.dataset_processed[p] if i.batch_idx in batch_idx_round]
                            print('pert', p , ': len of batch cells for round ', round, ' is ', len(data_load))
                            found = True
                            break
                    if not found:
                ...
                if eval_mode:
                    if p != 'ctrl':
                        cell_graphs.extend(data_load)
                else:
                    cell_graphs.extend(data_load)

        print_sys("Finished creating dataloaders....")
        if eval_mode:
            return DataLoader(cell_graphs, batch_size=batch_size, shuffle=shuffle, num_workers = num_workers, pin_memory = True)
        else:
            return DataLoader(cell_graphs, batch_size=batch_size, shuffle=shuffle, 
    ...
    def get_dataloader(self, batch_size, test_batch_size = None):
        ...
        if self.split == 'no_split':
            ...
            if self.split == 'active':
                train_pert = np.array(self.set2conditions['train'])
                np.random.seed(1)
                np.random.shuffle(train_pert)
                train_train_pert = train_pert[:int(0.9*len(train_pert))]
                train_val_pert = train_pert[int(0.9*len(train_pert)):]
                print('Length of training perts: ' + str(len(train_train_pert)))
                print('Length of validation perts: ' + str(len(train_val_pert)))
                print('Length of testing perts: ' + str(len(self.set2conditions['test'])))

                split2pert = {
                    'train': train_train_pert,
                    'val': train_val_pert,
                    'test': self.set2conditions['test']
                }
                for i,j in split2pert.items():
                    cell_graphs[i] = []
                    for p in j:
                        cell_graphs[i].extend(self.dataset_processed[p])
            ...
            # Set up dataloaders
            num_workers = 0
            print('Using number of workers: ' + str(num_workers))
    ...
    def create_cell_graph(self, X, y, de_idx, pert, pert_idx=None, batch=None):
        ...
        return Data(x=feature_mat, pert_idx=pert_idx,
                    ...
                    de_idx=de_idx, pert=pert, batch_idx = batch)
    ...
    def create_cell_graph_dataset(self, split_adata, pert_category,
        ...
        if 'batch' in adata_.obs.columns:
            #print('batch info is available!')
            batch_info = np.array(adata_.obs.batch.values)
        else:
            batch_info = None
...
## given a list of perts, for each pert, find all cells in this pert, construct X-Y pairs.
## x=feature_mat, pert_idx=pert_idx, y=torch.Tensor(y.reshape(1, -1)), de_idx=de_idx, pert=pert