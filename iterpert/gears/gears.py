...
from .inference import evaluate, compute_metrics, deeper_analysis, \
                  non_dropout_analysis, compute_synergy_loss, eval_analysis_fast, \
                    get_latent_emb, evaluate_mean_ctrl
...
class GEARS:
    def __init__(self, pert_data, 
        ...
        self.ctrl_mean = pert_data.ctrl_mean
    ...
    def model_initialize(self, hidden_size = 64,
        ...
        self.config['mean_expression'] = torch.tensor(self.adata[self.adata.obs.condition.isin(self.set2conditions['train'])].X.mean(axis = 0)).to(self.device)
    ...
    def predict_from_loader(self, dataloader, detail_eval = False):
        ...
        s = time.time()    
        if self.fix_evaluation:
            print('Using modified evaluation scheme...')
            res = evaluate_mean_ctrl(dataloader, self.best_model, self.device, self.ctrl_mean)
        else:
            res = evaluate(dataloader, self.best_model,
        ...
        if detail_eval:
            ...
            #s = time.time()
            #print('Starting deeper analysis...')
            #out = deeper_analysis(self.adata, res)
            #print('Deeper analysis finished with time ' + str((time.time() - s)/60) + ' min')
            #s = time.time()
            #out_non_dropout = non_dropout_analysis(self.adata, res)
            #print('non_dropout_analysis finished with time ' + str((time.time() - s)/60) + ' min')
    ...
    def train(self, epochs = 20, 
        ...
        self.valid_perts = dataloader['valid_pert']
        self.train_perts = dataloader['train_pert']
        self.train_valid_perts = dataloader['train_valid_pert']
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                #print('Getting a new batch takes ' + str(time.time() - s) + ' s!')
                batch.to(self.device)
                optimizer.zero_grad()
                y = batch.y
                if self.config['uncertainty']:
                    ...
                    #print('model forwarding takes ' + str(time.time() - s) + ' s!')
                    #s = time.time()
                    ...
                    #print('computing loss takes ' + str(time.time() - s) + ' s!')
                ...
                #s = time.time()
                loss.backward()
                #print('loss backward takes ' + str(time.time() - s) + ' s!')
                #s = time.time()
                nn.utils.clip_grad_value_(self.model.parameters(), clip_value=1.0)
                optimizer.step()
                #print('optimizer step takes ' + str(time.time() - s) + ' s!')
            ...
            print('this epoch takes ' + str(time.time() - s) + ' s!')
            scheduler.step()
            #s = time.time()
            # Evaluate model performance on train and val set
            #train_res = evaluate(train_loader, self.model,
            #                     self.config['uncertainty'], self.device)
            #print('per epoch train-evaluation takes ' + str(time.time() - s) + ' s!')
            s = time.time()

            if self.fix_evaluation:
                val_res = evaluate_mean_ctrl(val_loader, self.model, self.device, self.ctrl_mean)
            else:
                val_res = evaluate(val_loader, self.model,
                                 self.config['uncertainty'], self.device)
            print('per epoch evaluation takes ' + str(time.time() - s) + ' s!')
            s = time.time()
            #train_metrics, _ = compute_metrics(train_res)
            val_metrics, _ = compute_metrics(val_res)
            print('per epoch metric computing takes ' + str(time.time() - s) + ' s!')
            ...
            if self.wandb:
                metrics = ['mse', 'pearson']
                for m in metrics:
                    self.wandb.log({'val_'+m: val_metrics[m],
                               'val_de_'+m: val_metrics[m + '_de']})
                    self.wandb.log({'epoch with best val mse de': best_epoch})
