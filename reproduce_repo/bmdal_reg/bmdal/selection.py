...
class IterativeSelectionMethod(SelectionMethod):
    ...
    def select(self, batch_size: int) -> torch.Tensor:
        ...
        if self.with_train:
            ...
            print(f'Added {len(self.train_features)} train samples to selection', flush=True)

        for i in range(batch_size):
            next_idx = self.get_next_idx()
            if next_idx is None:
                print('next_idx is None')
            if  next_idx < 0:
                print('next_idx <0')
            if next_idx >= len(self.pool_features):
                print('next_idx >= len(self.pool_features)')
            if self.selected_arr[next_idx]:
                print('self.selected_arr[next_idx]')
...
class MaxDetSelectionWithPrior(IterativeSelectionMethod):
    """
    Implements the kernel-space version of MaxDet for Batch Active Learning.
    """
    def __init__(self, pool_features: Features, train_features: Features, sel_with_train: bool = False,
                 noise_sigma: float = 0.0, prior = None, **config):
        """
        :param pool_features: Features on the pool set.
        :param train_features: Features on the train set.
        :param sel_with_train: If True, TP-mode is used instead of P-mode. This is False by default.
        ...
        #self.diag = self.features.get_kernel_matrix_diag() + self.noise_sigma**2
        #if prior:
        print(torch.diag(prior, 0))
        self.diag = torch.diag(prior, 0) + self.noise_sigma**2
        self.prior = prior
        self.l = None
    ...
    def add(self, new_idx: int):
        ...
        #mat_col = self.features[new_idx].get_kernel_matrix(self.features).squeeze(0)
        mat_col = self.prior[new_idx].squeeze() ## pick the right index and its simialrity with all other samples
...
class MaxDistSelectionMethodwithPrior(IterativeSelectionMethod):
    """
    Implements the MaxDist selection method for Batch Active Learning.
    """
    def __init__(self, pool_features: Features, train_features: Features, sel_with_train: bool = True,  prior = None, 
                 **config):
        super().__init__(pool_features=pool_features, train_features=train_features,
                         sel_with_train=sel_with_train, **config)
        self.min_sq_dists = np.Inf * torch.ones(self.pool_features.get_n_samples(), device=pool_features.get_device())
        self.prior = prior
        self.diag = torch.diag(self.prior, 0)
    def get_scores(self) -> torch.Tensor:
        return self.min_sq_dists
    ...
    def get_next_idx(self) -> Optional[int]:
        if self.n_added == 0:
            # no point added yet, take point with largest norm
            #return torch.argmax(self.pool_features.get_kernel_matrix_diag()).item()
            return torch.argmax(torch.diag(self.prior, 0)[:len(self.pool_features)]).item()
        scores = self.get_scores().clone()
        scores[self.selected_idxs] = -np.Inf
        idx = torch.argmax(scores).item()
        print('Next idx:', idx, '- Value:', scores[idx].item())
    ...
    def add(self, new_idx: int):
        diag = self.diag[new_idx]
        print(diag)
        other_diag = self.diag[:len(self.pool_features)]
        print(other_diag)
        print(other_diag.shape)
        kernel_matrix = self.prior[new_idx, :len(self.pool_features)]
        print(kernel_matrix.shape)
        sq_dists = diag + other_diag - 2*kernel_matrix
...
class DirichletSelectionMethod(IterativeSelectionMethod):
    ...
    Implements the MaxDist selection method for Batch Active Learning.
    """
    def __init__(self, pool_features: Features, train_features: Features, sel_with_train: bool = True, lamb = 2, round = 1, prior = None,
                 **config):
        super().__init__(pool_features=pool_features, train_features=train_features,
                         sel_with_train=sel_with_train, **config)
        self.min_sq_dists = np.Inf * torch.ones(self.pool_features.get_n_samples(), device=pool_features.get_device())
        ...
        if self.prior is not None:
            self.diag = torch.diag(self.prior, 0)
            diag = torch.diag(self.prior, 0)[:len(self.pool_features)]
            other_diag = torch.diag(self.prior, 0)
            kernel_matrix = self.prior[:len(self.pool_features)]
            sq_dist_pool = diag[:, None] + other_diag[None, :] - 2*kernel_matrix
            sq_dist_pool = sq_dist_pool.squeeze(0)
        else:
            sq_dist_pool = self.pool_features.get_sq_dists(self.features).squeeze(0)

        sq_dist_pool = sq_dist_pool.detach().cpu().numpy()
        num_items = sq_dist_pool.shape[0]
        K = 20
        # Compute typicality for each item
        typicality_scores = np.zeros(num_items)
        for i in range(num_items):
            # Get distances for the current item
            distances = sq_dist_pool[i]
            
            # Get the K smallest distances (excluding the distance to itself)
            k_nearest_distances = np.partition(distances, K+1)[:K+1]  # +1 because the smallest distance will be 0 (distance to itself)
            k_nearest_distances = k_nearest_distances[k_nearest_distances != 0]  # Exclude distance to itself
            
            # Compute the average distance
            avg_distance = np.mean(k_nearest_distances)
            typicality_scores[i] = 1/avg_distance

        self.typicality_scores = torch.tensor(typicality_scores).to(pool_features.get_device())
        print('Typicality score:')
        print(self.typicality_scores)
        self.lamb = lamb/round
        print('Using lambda: ' + str(self.lamb))
    def get_scores(self) -> torch.Tensor:
        return self.min_sq_dists
    ...
    def get_next_idx(self) -> Optional[int]:
        ...
        #try:
        print('scale:')
        print(torch.mean(self.min_sq_dists)/torch.mean(self.typicality_scores))
        alpha_prior = self.lamb * torch.mean(self.min_sq_dists)/torch.mean(self.typicality_scores) * self.typicality_scores
        alpha_posterior = alpha_prior + self.min_sq_dists
        print('alpha posterior:')
        print(alpha_posterior)
        print('data:')
        print(self.min_sq_dists)
        print('alpha prior:')
        print(alpha_prior)
        print('<=0 numbers: ', sum(alpha_posterior <= 0))
        alpha_posterior = torch.where(alpha_posterior <= 0, torch.tensor(1e-10), alpha_posterior)
        dirichlet_dist = Dirichlet(alpha_posterior)

        samples = 0 
        while True:
            idx = torch.argmax(dirichlet_dist.sample()).item()
            if not self.selected_arr[idx]:
                break
            else:
                samples += 1
                print('already selected... we select again with total tries: ' + str(samples))
                if samples > 10:
                    raise ValueError
                
        print('Next idx:', idx, '- Min-SQ Distance:', self.min_sq_dists[idx].item(), '- Typicality:', alpha_prior[idx].item())
        return idx
        #except RuntimeError:
